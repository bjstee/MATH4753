...
myncurve = function(mu, sigma, a = 0){
  ...
  ycurve=stats::dnorm(xcurve,mean=mu,sd=sigma) # Y values corresponding t0 the x values
  # Fill in the polygon with the given vertices
  graphics::polygon(c(mu-3*sigma,xcurve,a),c(0,ycurve,0),col="lightblue")
  # Find lower tail probability
  prob=stats::pnorm(a,mean=mu,sd=sigma) # pnorm calculates lower tail, so -inf bound holds
  prob=round(prob,4)
